...
from src.AutoDetailer import SAM, ADetailer, bbox, SEGS
from src.AutoEncoders import VariationalAE
from src.clip import Clip
from src.sample import sampling

from src.Utilities import util
from src.UltimateSDUpscale import USDU_upscaler, UltimateSDUpscale
...
from src.sample.multiscale_presets import MULTISCALE_PRESETS, get_preset_parameters

Downloader.CheckAndDownload()

files = glob.glob("./include/checkpoints/*.safetensors")
loras = glob.glob("./include/loras/*.safetensors")
loras += glob.glob("./include/loras/*.pt")
...
class App(tk.Tk):
            ...
            with torch.inference_mode():
                ...
                self.checkpointloadersimple_241 = (
                    self.checkpointloadersimple.load_checkpoint(
                        ckpt_name="./include/checkpoints/" + self.ckpt
                    )
                )
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                try:
                    ...
                    print(
                        "loading",
                        self.lora_selection.get().replace("./include/loras/", ""),
                    )
                except:
                    loraloader_274 = checkpointloadersimple_241
                try:
    ...
    def display_most_recent_image(self) -> None:
        ...
        image_files = glob.glob("./output/Classic/*")
        image_files += glob.glob("./output/Adetailer/*")
        image_files += glob.glob("./output/Flux/*")
        image_files += glob.glob("./output/HiresFix/*")
        image_files += glob.glob("./output/Img2Img/*")
...
if __name__ == "__main__":
    from src.user.app_instance import app
